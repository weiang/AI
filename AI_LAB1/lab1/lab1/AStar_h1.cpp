...
{	
	...
	for (int i = 0; i != 5; i ++) {
		for (int j = 0; j != 3; j ++) {
			if (node -> status[i][j] != goalStatus[i][j])
				result ++;
		}
	}
	return result;
